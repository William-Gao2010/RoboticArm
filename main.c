...
void x()
{
		if((number[3]>(0x0a+10))){DegreeData[0]+=5;	UpdatePwm(1,DegreeData[0]);	Delay1ms(100);}
...
void y()
	{
		if((number[2]>(0x09+10))){DegreeData[1]+=5;UpdatePwm(4,DegreeData[1]);	Delay1ms(100);}
		if(DegreeData[1]>0x80){DegreeData[1]=0x82;}
		if((number[2]<(0x09-10))){DegreeData[1]-=5;UpdatePwm(4,DegreeData[1]);	Delay1ms(100);}
		if(DegreeData[1]<0x26){DegreeData[1]=0x21;}
}
